...
import { BotMessageSquare, LayoutDashboard, Package, ShoppingBag, BarChart3, Settings, MessageSquare, Warehouse, Truck, Bell, Shapes, Boxes, FileText, ChevronLeft, PlusCircle, Trash2, DollarSign, CalendarIcon } from "lucide-react";
...
const quoteItemSchema = z.object({
  ...
  product_name: z.string(), // For display, will be set on product selection
...
const createQuoteSchema = z.object({
  ...
  quote_number: z.string().optional(), // Could be auto-generated by backend
  ...
  // subtotal, tax, total_amount will be calculated
});
...
export default function CreateNewQuotePage() {
  ...
  const form = useForm<CreateQuoteFormValues>({
    ...
    defaultValues: {
      buyer_name: "",
      quote_number: `QT-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 10000)).padStart(4, '0')}`, // Suggested
  ...
  // Watch items to recalculate totals on change
  useEffect(() => {
    const subscription = form.watch((value, { name, type }) => {
      if (name?.startsWith("items")) {
        form.trigger(); // To re-render and show new totals
      }
    });
  ...
  const handleProductSelect = (itemIndex: number, selectedProductId: string) => {
    ...
    if (product) {
      updateItem(itemIndex, {
        ...
        quoted_price: product.base_price, // Default to base price
      });
      form.trigger(`items.${itemIndex}.quoted_price`); // Trigger revalidation/update if needed
    }
  };
  ...
  const onSubmit = async (values: CreateQuoteFormValues) => {
    ...
    // Optionally redirect: router.push('/seller/dashboard/quotes');
    setIsSubmitting(false);
  };
